...
class ShareManagerInterface(object):
    ...
    def on_network_block(self, block_height):
        '''Prints when there's new block coming from the network (possibly new round)'''
        self.block_height = block_height
        pass
    
    def on_submit_share(self, worker_name, block_header, block_hash, difficulty, pool_share, timestamp, is_valid, ip, invalid_reason, share_diff, job_id):
    ...
    def on_submit_aux_block(self, is_accepted, worker_name, block_header, block_hash, timestamp, ip, share_diff):
        log.info("MERGED BLOCK %s %s" % (block_hash, 'ACCEPTED' if is_accepted else 'REJECTED'))
        dbi.found_aux_block([worker_name, block_hash, -1, timestamp, is_accepted, ip, 'MERGED BLOCK', share_diff ])
