...
def setup(on_startup):
    ...
    *before* you call setup() in the launcher script.'''
    ...
    log.info("Connecting to merged coin daemon(s)...")
    for chain in range(len(aux_rpc.conns)):
        try:
            result = (yield aux_rpc.conns[chain].check_height())
        except ConnectionRefusedError, e:
            log.error("Connection refused while trying to connect to the coin (are your COIN settings correct?)")
            reactor.stop()
            break
        ...
        try:
            result = (yield aux_rpc.conns[chain].getauxblock())
            if isinstance(result, dict):
                # litecoind implements version 1 of getblocktemplate
                print result
                #break
            ...
            if isinstance(e[2], str):
                try:
                    if isinstance(json.loads(e[2])['error']['message'], str):
                        error = json.loads(e[2])['error']['message']
                    if error == "Method not found":
                        log.error("Coin daemon does not support getauxblock!!!")
                        reactor.stop()
                    elif "downloading blocks" in error:
                        log.error("Coin downloading blockchain... will check back in 30 sec")
                        time.sleep(29)
                    ...
                    log.error("Failed Connect(HTTP 500 or Invalid JSON), Check Username and Password!")
                    reactor.stop()
        time.sleep(0)  # If we didn't get a result or the connect failed
        
    log.info('Connected to merged coin daemon(s)! YES!')
    log.info('Begining to load Address and Module Checks!')
