...
class SimpleCoinbaser(object):
    ...
    def address_check(self, result):
        if result['isvalid'] and result['ismine']:
            self.is_valid = True
            log.info("Coinbase address '%s' valid" % self.address)
            ...
            if not self.on_load.called:
             ...
             self.is_valid = True
             log.warning("Coinbase address '%s' VALID NONLOCAL WALLET!!!" % self.address)
             if 'pubkey' in result:
               log.debug("PubKey = %s " % result['pubkey'])
               self.pubkey = result['pubkey']
             if not self.on_load.called:
            ...
            self.is_valid = False
            log.error("Coinbase address '%s' is INVALID!" % self.address)
