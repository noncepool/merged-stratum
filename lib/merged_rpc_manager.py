...
import lib.logger
log = lib.logger.get_logger('merged_rpc_manager')

from lib.bitcoin_rpc import BitcoinRPC


class MergedRPCManager(object):
    ...
    def __init__(self,host=None,port=None,user=None,passwd=None):
        ...
        if host is None or port is None or user is None or passwd is None:
            self.conns[0] = BitcoinRPC(settings.MERGED_HOST,
                                       settings.MERGED_PORT,
                                       settings.MERGED_USER,
                                       settings.MERGED_PASSWORD)
            self.curr_conn = 0
            for x in range (1,99):
                if hasattr(settings, 'MERGED_HOST_' + str(x)) and hasattr(settings, 'MERGED_PORT_' + str(x)) and hasattr(settings, 'MERGED_USER_' + str(x)) and hasattr(settings, 'MERGED_PASSWORD_' + str(x)):
                    self.conns[len(self.conns)] = BitcoinRPC(settings.__dict__['MERGED_HOST_' + str(x)],
                                                             settings.__dict__['MERGED_PORT_' + str(x)],
                                                             settings.__dict__['MERGED_USER_' + str(x)],
                                                             settings.__dict__['MERGED_PASSWORD_' + str(x)])
        else:
